...
const EXECJS_RUNTIME_API_DOCS = [
  ...
  '- captureView({ viewId }) => Promise<{ base64: string; mimeType: "image/png" }>',
  '- getViewConsoleLogs({ viewId, since?, limit? }) => Promise<Array<{ level: string; message: string; timestamp: number }>>',
  '- execViewJs({ viewId, code, timeoutMs? }) => Promise<any>',
].join('\n')
...
const CODE_SYSTEM_PROMPT = [
  ...
  '- Use captureView({ viewId }) for screenshots of open views.',
  '- Use execViewJs({ viewId, code, timeoutMs? }) for interactions inside open view runtimes.',
  '- Use runSql with confirmed=true only when a mutation is intentional and explicitly justified.',
  '',
  '## [agentviews]',
  'Agent views are DB-backed UI views stored in the agent database table "views" (columns include id, name, content, updated_at).',
  'They are rendered as isolated webview runtimes via agentview://view/<viewId> when a view tab is opened in the app.',
  '',
  'How to work with agent views:',
  '- Discover views with runSql on target="agent" (for example: SELECT id, name, updated_at FROM views ORDER BY updated_at DESC).',
  '- Use the views.id value as viewId for captureView, getViewConsoleLogs, and execViewJs.',
  '- These diagnostics APIs require an open view runtime; if a view is not open, ask the user to open that view tab first.',
  '- If multiple tabs share the same viewId, diagnostics resolve to the active tab first, then focused/recent fallback.',
  '- Typical flow: captureView for visual state, getViewConsoleLogs for runtime signals, execViewJs for targeted inspection/actions.',
  '',
  'Inside an agent view runtime:',
  '- execViewJs executes in that view\'s webview context (window/document are from the selected view runtime).',
  '- The same capabilities are available in the view runtime with the same method names (runSql, read, write, edit, ls, mkdir, remove, find, grep, captureView, getViewConsoleLogs, execViewJs).',
  '- In view code, call them as window.electronAgentTools.<method>(...) (file methods use positional args; runSql and view diagnostics use object params).',
