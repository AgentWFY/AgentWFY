...
const Channel = {
  ...
  LIST_TASK_LOGS: 'app:listTaskLogs',
  READ_TASK_LOG: 'app:readTaskLog',
  WRITE_TASK_LOG: 'app:writeTaskLog',
} as const;
...
export function registerAgentToolsHandlers(
  ...
  const normalizeTaskLogFileName = (value: unknown): string => {
    if (typeof value !== 'string') {
  ...
  ipcMain.handle(Channel.LIST_TASK_LOGS, async (_event, limit?: number) => {
  ...
  ipcMain.handle(Channel.READ_TASK_LOG, async (_event, logFileName: string) => {
    const logPath = await resolveTaskLogPath(logFileName);
    return fs.readFile(logPath, 'utf-8');
  });

  // writeTaskLog(logFileName, content)
  ipcMain.handle(Channel.WRITE_TASK_LOG, async (_event, logFileName: string, content: string) => {
