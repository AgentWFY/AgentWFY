...
App settings are stored in the `config` table. Each setting can be set at agent level (in the DB) or globally (in the app's Electron store). The user can manage settings via the command palette.
...
All values are stored as strings — no JSON encoding. Numbers and booleans are their string form (e.g. `'8080'`, `'true'`).
