...
```javascript
const { agentId } = await spawnAgent('Analyze the data and return a JSON summary.')
const { response } = await waitFor(`agent:response:${agentId}`, 120000)
...
const { agentId } = await spawnAgent('You are a trading assistant. Answer questions about portfolio data.')
await waitFor(`agent:response:${agentId}`, 120000) // wait for initial response

// Send follow-up messages (multi-turn conversation)
await sendToAgent(agentId, 'What is the total P&L for March?')
const { response } = await waitFor(`agent:response:${agentId}`, 120000)
console.log(response)

// Continue the conversation — full history is preserved
await sendToAgent(agentId, 'Break it down by strategy.')
const { response: r2 } = await waitFor(`agent:response:${agentId}`, 120000)
console.log(r2)
```
