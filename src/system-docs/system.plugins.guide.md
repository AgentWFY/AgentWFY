...
const result = await requestInstallPlugin({ packagePath: '.tmp/my-plugin.plugins.awfy' })
console.log(result.installed) // ['my-plugin'] or []

// Toggle a plugin off
const toggle = await requestTogglePlugin({ pluginName: 'my-plugin' })
console.log(toggle) // { toggled: true, enabled: false } or { toggled: false }

// Uninstall a plugin
const removed = await requestUninstallPlugin({ pluginName: 'my-plugin' })
