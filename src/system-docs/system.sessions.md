...
`spawnSession({ prompt, providerId?, providerOptions? })` → `{ sessionId }` — spawn a session. It runs independently with its own execJs context and the same host APIs. The `sessionId` is a stable public session id. Pass `providerId` to use a specific provider instead of the default. Pass `providerOptions` to override provider behavior for this session (e.g. `{ model: 'gpt-4o' }` to use a different model).
