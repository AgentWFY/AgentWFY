// Mobile-only bottom navigation: Chat / Tasks / Views. Parent (<awfy-app>) sets
// the screen attribute; clicking a tab dispatches set-screen back up.

import { dispatch, type Screen } from '../events.js'
import { ICON_CHAT, ICON_GRID, ICON_TASKS } from './icons.js'
...
export class TlBottomTabs extends HTMLElement {
  ...
  connectedCallback() {
    ...
    this.innerHTML = `
      ...
      <button type="button" class="bottom-tab-btn" data-screen="tasks">${ICON_TASKS}<span>Tasks</span></button>
    ...
    this.querySelectorAll<HTMLButtonElement>('.bottom-tab-btn').forEach((btn) => {
      btn.addEventListener('click', () => {
        ...
        if (t === 'chat' || t === 'tasks' || t === 'views') dispatch('set-screen', { screen: t })
      })
    })
    this.applyActive()
  }
