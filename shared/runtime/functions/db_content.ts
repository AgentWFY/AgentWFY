...
export async function dbWrite(runtimeRoot: string, dbPath: DbPath, content: string): Promise<string> {
  ...
  await runAgentDbSql(runtimeRoot, {
    sql: spec.sql,
...
export async function dbLs(runtimeRoot: string, dbPath: DbPath, limit?: number): Promise<string[]> {
  if (!dbPath.table) {
...
export async function dbRemove(runtimeRoot: string, dbPath: DbPath): Promise<void> {
