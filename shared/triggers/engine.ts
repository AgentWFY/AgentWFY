...
export class TriggerEngine {
  ...
  private async doReload(): Promise<void> {
    ...
    const runtimeRoot = this.deps.getRuntimeRoot();

    let rows: TriggerRow[];
    try {
      const raw = await runAgentDbSql(runtimeRoot, {
  ...
  private async startHttpServer(): Promise<void> {
    const runtimeRoot = this.deps.getRuntimeRoot();
    cleanStaleLockfile(runtimeRoot);
    this.httpApi = await startHttpApi({ getRuntimeRoot: this.deps.getRuntimeRoot, preferredPort: this.deps.getPreferredPort() });
    writeLockfile(runtimeRoot, this.httpApi.port());
  }
  ...
  private stopHttpServer(): void {
    ...
    this.httpApi = null;
    removeLockfile(this.deps.getRuntimeRoot());
  }
  ...
  private ensureFileWatcher(dir: string): () => void {
    const runtimeRoot = this.deps.getRuntimeRoot();
    const absDir = resolveInsideRoot(runtimeRoot, dir);
