...
export class PluginRegistry {
  ...
  loadPlugin(row: { name: string; title: string; description: string; version: string; code: string }): void {
    const assetsDir = path.join(this.runtimeRoot, '.agentwfy', 'plugin-assets', row.name)
    ...
    try {
      ...
      const api: PluginApi = {
        ...
        getConfig: (name: string, fallback?: unknown): unknown => {
          return getConfigValue(this.runtimeRoot, name, fallback)
        },
        setConfig: (name: string, value: unknown): void => {
          setAgentConfig(this.runtimeRoot, name, value)
        },
  ...
  unloadPlugin(name: string): void {
    ...
    handleProviderFallback(this.runtimeRoot, removedProviders)
...
function handleProviderFallback(runtimeRoot: string, removedProviders: string[]): void {
  if (removedProviders.length === 0) return
  const currentProvider = getConfigValue(runtimeRoot, SystemConfigKeys.provider) as string | undefined
  if (currentProvider && removedProviders.includes(currentProvider)) {
    clearAgentConfig(runtimeRoot, SystemConfigKeys.provider)
  }
}
