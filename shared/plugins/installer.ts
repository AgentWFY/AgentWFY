...
export function installFromPackage(runtimeRoot: string, packagePath: string): { installed: string[] } {
  return installPackageData(runtimeRoot, readValidatedPackage(packagePath))
}

export function installPackageData(runtimeRoot: string, { plugins, docs, views, config, assets }: PackageData): { installed: string[] } {
  ...
  for (const asset of assets) {
    const filePath = path.join(runtimeRoot, '.agentwfy', 'plugin-assets', asset.name)
...
export function uninstallPlugin(runtimeRoot: string, pluginName: string): void {
  const db = getOrCreateAgentDb(runtimeRoot)
  db.uninstallPlugin(pluginName)

  // Remove plugin assets directory
  const assetsDir = path.join(runtimeRoot, '.agentwfy', 'plugin-assets', pluginName)
  try {
